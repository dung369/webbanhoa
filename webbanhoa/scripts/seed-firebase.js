...
async function main() {
  ...
  if (!adminEmail || !adminPassword) {
    console.error(
      "Please set SEED_ADMIN_EMAIL and SEED_ADMIN_PASSWORD as environment variables"
    );
    process.exit(1);
  }
  ...
  if (Array.isArray(seed.users)) {
    ...
    seed.users.forEach((u) => {
      ...
      const doc = Object.assign({}, u, {
        createdAt: admin.firestore.Timestamp.fromDate(new Date(u.createdAt)),
      });
      batch.set(docRef, doc, { merge: true });
    });
    await batch.commit();
  ...
  if (Array.isArray(seed.orders)) {
    ...
    seed.orders.forEach((o) => {
      ...
      const doc = Object.assign({}, o, {
        createdAt: admin.firestore.Timestamp.fromDate(new Date(o.createdAt)),
      });
      batch.set(docRef, doc, { merge: true });
    });
    await batch.commit();
  ...
  if (Array.isArray(seed.customers)) {
    ...
    seed.customers.forEach((cust) => {
      ...
      const doc = Object.assign({}, cust, {
        createdAt: admin.firestore.Timestamp.fromDate(
          new Date(cust.createdAt || Date.now())
        ),
      });
      batch.set(docRef, doc, { merge: true });
    });
  ...
  console.log("\nSeeding complete.");
  console.log(
    "IMPORTANT: Change the admin password after first login and do NOT commit credentials to source control."
  );
  process.exit(0);
}
