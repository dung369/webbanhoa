      ...
      <div className="w-96 bg-white border rounded-lg shadow-lg overflow-hidden">
        <div className="flex items-center justify-between p-3 border-b">
          <div className="flex items-center gap-2">
            ...
            <div className="font-semibold">Chat với khách</div>
          </div>
          <button
            ...
            {conversations.map((c) => (
              <div
                ...
                className={`p-3 cursor-pointer ${
                  c.id === selected ? "bg-rose-50" : "hover:bg-slate-50"
                }`}
                onClick={() => setSelected(c.id)}
              >
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-rose-100 flex items-center justify-center text-rose-700 text-sm">
                    {(c.items[0]?.name || "K")
                      .split(" ")
                      .map((s: any) => s[0])
                      .slice(0, 2)
                  ...
                  <div className="flex-1">
                    <div className="font-medium text-sm">
                      {c.items[0]?.name || c.items[0]?.email || c.id}
                    </div>
                    <div className="text-xs text-slate-500">
                      {c.items[c.items.length - 1]?.createdAt
                        ?.slice(0, 19)
                        .replace("T", " ")}
                    </div>
                  </div>
                ...
                selectedConv.items.map((m: any, i: number) => (
                  <div
                    ...
                    className={`p-2 rounded ${
                      m.role === "admin"
                        ? "bg-rose-100 text-rose-900 self-end ml-auto"
                        : "bg-slate-100 text-slate-800"
                    }`}
                  >
                    <div className="text-xs font-medium">
                      {m.name || m.email || m.senderId || m.id}
                    </div>
