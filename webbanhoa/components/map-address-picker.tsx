  ...
  function extractComponents(place: any): AddressComponents {
    ...
    place.address_components.forEach((c: any) => {
      ...
      if (
        ...
        comps.sublocality = c.long_name;
      if (c.types.includes("locality")) comps.locality = c.long_name;
      if (c.types.includes("administrative_area_level_1"))
        comps.administrative_area_level_1 = c.long_name;
  ...
  function initMap() {
    ...
    marker.addListener("dragend", () => {
      ...
      geocoder.geocode({ location: { lat, lng } }, (results: any) => {
        const formatted =
          (results && results[0] && results[0].formatted_address) || "";
        const comps =
          results && results[0] ? extractComponents(results[0]) : {};
  ...
  const withinDelivery =
    picked && picked.lat != null && picked.lng != null
      ? haversine(
          STORE_CENTER.lat,
          STORE_CENTER.lng,
          picked.lat as number,
          picked.lng as number
        ) <= DELIVERY_MAX_KM
      : true;
  const distanceKm =
    picked && picked.lat != null && picked.lng != null
    ...
    <div className="space-y-2">
      <div>
        <input
          ...
          onBlur={() => {
            ...
            if (
              inputValue.trim() &&
              (!picked || picked.formatted !== inputValue.trim())
            ) {
              const newPicked = {
                lat: picked?.lat ?? null,
                lng: picked?.lng ?? null,
      ...
      <div className="text-sm text-rose-700">
        {picked ? (
          <div>
            <div>
              <strong>Địa chỉ:</strong> {picked.formatted}
            </div>
            {picked.lat != null && picked.lng != null ? (
              <div>
                <strong>Tọa độ:</strong> {picked.lat.toFixed(6)},{" "}
                {picked.lng.toFixed(6)}
              </div>
            ) : null}
            <div>
              <strong>Khoảng cách:</strong> {distanceKm.toFixed(2)} km
            </div>
            <div>
              <strong>Phí giao:</strong>{" "}
              {withinDelivery
                ? computeShipping(distanceKm).toLocaleString("vi-VN") + "đ"
                : "Ngoài vùng giao hàng"}
            </div>
          </div>
        ) : null}
      </div>
