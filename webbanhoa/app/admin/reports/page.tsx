...
export default function AdminReportsPage() {
  ...
  const productsSoldCount = orders.reduce((s, o) => {
    ...
    return (
      s +
      items.reduce(
        (ss: number, it: any) => ss + (Number(it.quantity || it.qty || 1) || 0),
        0
      )
    );
  }, 0);
  ...
  const productCategories = (() => {
    try {
      const catMap: Record<
        string,
        { name: string; value: number; revenue: number; color: string }
      > = {};
      orders.forEach((o) => {
        const items = Array.isArray(o.items) ? o.items : [];
        items.forEach((it: any) => {
          ...
          const revenue =
            (Number(it.price || it.unitPrice || prod?.price || 0) || 0) *
  ...
  const topProducts = (() => {
    const map: Record<
      string,
      { name: string; sold: number; revenue: number; growth: number }
    > = {};
    orders.forEach((o) => {
      const items = Array.isArray(o.items) ? o.items : [];
      items.forEach((it: any) => {
  ...
  const customerSegments = (() => {
    ...
    const newCustomers = users.filter(
      (u) => toDate(u.createdAt) >= daysAgo(30)
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        count: regular,
        percentage: totalCustomers
          ? +((regular / totalCustomers) * 100).toFixed(1)
          : 0,
        avgSpent: 0,
      },
    ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {(() => {
            ...
            const totalRevenuePrevMonth = orders.reduce((s, o) => {
              ...
              const isPrev =
                m === (prevMonth < 0 ? 11 : prevMonth) && y === prevYear;
            ...
            const newCustomersPrevMonth = users.filter((u) => {
              const d = toDate(u.createdAt);
              return (
                d.getMonth() === (prevMonth < 0 ? 11 : prevMonth) &&
            ...
            const productsSoldPrevMonth = orders.reduce((s, o) => {
              ...
              const m = d.getMonth();
              const y = d.getFullYear();
              if (!(m === (prevMonth < 0 ? 11 : prevMonth) && y === prevYear))
                return s;
              const items = Array.isArray(o.items) ? o.items : [];
              return (
                s +
                items.reduce(
                  (ss: number, it: any) =>
                    ss + (Number(it.quantity || it.qty || 1) || 0),
                  0
                )
              );
            }, 0);
              ...
              <Card className="border-slate-200">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div
                      className={`w-12 h-12 ${bgClass} rounded-full flex items-center justify-center`}
                    >
                      {icon}
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <p className="text-sm text-slate-600">
                        khách hàng ({segment.percentage}%)
                      </p>
                    </div>
                    <div>
                      <p className="text-lg font-medium text-rose-600">
                        {segment.avgSpent.toLocaleString("vi-VN")}đ
                      </p>
                      <p className="text-sm text-slate-600">
                        Chi tiêu trung bình
                      </p>
