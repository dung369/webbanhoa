...
export default function AdminDashboard() {
  ...
  const revenueData = (() => {
    try {
      ...
      orders.forEach((o) => {
        const d = o.createdAt && o.createdAt.toDate ? o.createdAt.toDate() : o.createdAt ? new Date(o.createdAt) : new Date()
  ...
  const orderStatusData = [
    ...
    { name: "Đang giao", value: orders.filter((o) => o.status === 'shipping').length, color: "#f59e0b" },
    { name: "Đã xác nhận", value: orders.filter((o) => o.status === 'confirmed').length, color: "#3b82f6" },
    { name: "Chờ xác nhận", value: orders.filter((o) => o.status === 'pending').length, color: "#ef4444" },
  ]

  const topProducts = (() => {
    try {
      const map: Record<string, { name: string; revenue: number; sold: number }> = {}
      orders.forEach((o) => {
  ...
  const recentOrders = orders.slice(0, 3).map((o) => ({ id: o.id, customer: (o.customerName || o.customer || o.userId) , total: o.total, status: o.status, date: o.createdAt && o.createdAt.toDate ? o.createdAt.toDate().toISOString().slice(0,10) : o.createdAt }))
  ...
  const [filterType, setFilterType] = useState<'orders'|'customers'|'products'>('orders')
  ...
  const loadProductsForDelete = async () => {
    try {
      if (firestore) {
        ...
        setDeleteProducts(snap.docs.map((d) => ({ id: d.id, ...(d.data() || {}) })))
      } else {
    ...
    try {
      if (firestore) {
        if (type === 'products') {
          const q = query(collection(firestore, 'products'), orderBy('createdAt', 'desc'))
          const snap = await getDocs(q)
          setFilterItems(snap.docs.map((d) => ({ id: d.id, ...(d.data() || {}) })))
        } else if (type === 'orders') {
        ...
        if (type === 'products') {
          ...
          if (res.ok) setFilterItems(payload && payload.orders ? payload.orders : [])
          else setFilterItems([])
        } else if (type === 'customers') {
          const res = await fetch('/api/customers')
          const payload = await res.json().catch(() => null)
          if (res.ok) setFilterItems(payload && payload.customers ? payload.customers : [])
          else setFilterItems([])
        }
      }
    } catch (e) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Hoa Tươi Việt - Quản lý cửa hàng</p>
              </div>
            </div>
      ...
      <div className="p-6">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-slate-900">{stats.totalRevenue.toLocaleString("vi-VN")}đ</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-2">
                    <TrendingUp className="w-4 h-4 text-green-500 mr-1" />
                    <span className="text-sm text-green-600">+{stats.productsChange}%</span>
                  </div>
                </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-8">
          ...
          <Card className="lg:col-span-2 border-slate-200">
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-slate-900">Doanh thu 7 ngày qua</CardTitle>
        ...
        <Card className="border-slate-200 mt-8">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <Dialog open={deleteDialogOpen} onOpenChange={(open) => { setDeleteDialogOpen(open); if (open) loadProductsForDelete() }}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>Chọn sản phẩm muốn xóa. Nếu Firestore được cấu hình thì sẽ xóa từ Firestore, ngược lại xóa từ local.</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3 mt-4">
                    <label className="block text-sm text-slate-700">Sản phẩm</label>
                    <select className="w-full border rounded p-2" value={selectedDeleteId || ''} onChange={(e) => setSelectedDeleteId(e.target.value)}>
                      <option value="">-- Chọn sản phẩm --</option>
                      {deleteProducts.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.title || p.name || `#${p.id}`} {p.images && p.images.length ? `— ${String(p.images[0]).split('/').pop()}` : ''}
                        </option>
                      ))}
                    </select>
                    {deleteMessage && <div className="text-sm text-slate-700">{deleteMessage}</div>}
                  </div>
                  <DialogFooter>
                    <Button variant="outline" onClick={() => { setDeleteDialogOpen(false); setSelectedDeleteId(null); setDeleteMessage(null) }}>Hủy</Button>
                    <Button onClick={async () => {
                      if (!selectedDeleteId) return setDeleteMessage('Vui lòng chọn một sản phẩm')
                      setDeleteLoading(true)
                      try {
                        if (firestore) {
                          await deleteDoc(doc(firestore, 'products', selectedDeleteId))
                          setDeleteMessage('Đã xóa sản phẩm (Firestore)')
                        } else {
                          const prev = deleteProducts
                          setDeleteProducts(prev.filter((p) => p.id !== selectedDeleteId))
                          const res = await fetch('/api/local-products', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: selectedDeleteId }) })
                          const payload = await res.json().catch(() => null)
                          if (res.ok) setDeleteMessage('Đã xóa sản phẩm (local)')
                          else {
                            setDeleteProducts(prev)
                            setDeleteMessage('Xóa thất bại: ' + (payload?.error || res.status))
                          }
                        }
                      } catch (e) {
                        // eslint-disable-next-line no-console
              ...
              <Dialog open={filterDialogOpen} onOpenChange={(open) => { setFilterDialogOpen(open); if (open) loadFilterItems(filterType) }}>
                <DialogTrigger asChild>
                  <></>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>Chọn loại dữ liệu để xóa (dùng cẩn thận). Trong môi trường local sẽ xóa từ file JSON.</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3 mt-4">
                    <label className="block text-sm text-slate-700">Loại dữ liệu</label>
                    <select className="w-full border rounded p-2" value={filterType} onChange={(e) => { const v = e.target.value as any; setFilterType(v); loadFilterItems(v) }}>
                      <option value="orders">Đơn hàng</option>
                      <option value="customers">Khách hàng</option>
                      <option value="products">Sản phẩm</option>
                    </select>

                    <label className="block text-sm text-slate-700">Bản ghi</label>
                    <select className="w-full border rounded p-2" value={filterSelectedId || ''} onChange={(e) => setFilterSelectedId(e.target.value)}>
                      <option value="">-- Chọn bản ghi --</option>
                      {filterItems.map((it) => (
                        <option key={it.id} value={it.id}>{(it.name || it.title || it.email || `#${it.id}`)}</option>
                      ))}
                    </select>
                  ...
                  <DialogFooter>
                    <Button variant="outline" onClick={() => { setFilterDialogOpen(false); setFilterSelectedId(null); setDeleteMessage(null) }}>Hủy</Button>
                    <Button onClick={async () => {
                      if (!filterSelectedId) return setDeleteMessage('Vui lòng chọn một bản ghi')
                      setFilterLoading(true)
                      try {
                        if (firestore) {
                          // Use Firestore deletion where applicable
                          if (filterType === 'products') await deleteDoc(doc(firestore, 'products', filterSelectedId))
                          if (filterType === 'orders') await deleteDoc(doc(firestore, 'orders', filterSelectedId))
                          if (filterType === 'customers') await deleteDoc(doc(firestore, 'users', filterSelectedId))
                          setDeleteMessage('Đã xóa (Firestore)')
                        } else {
                          let res
                          if (filterType === 'products') {
                            res = await fetch('/api/local-products', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: filterSelectedId }) })
                          } else if (filterType === 'orders') {
                            res = await fetch('/api/orders', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: filterSelectedId }) })
                          } else if (filterType === 'customers') {
                            res = await fetch('/api/customers', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: filterSelectedId }) })
                          }
                          ...
                          if (res && res.ok) {
                            ...
                            setDeleteMessage('Xóa thất bại: ' + (payload?.error || (res && res.status) || 'unknown'))
                          }
                        }
                      } catch (e) {
                        // eslint-disable-next-line no-console
                    ...
                    }}>{filterLoading ? 'Đang xóa...' : 'Xóa'}</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
