...
import { ArrowLeft, Search, Eye, Edit, Mail, Phone, Filter, Download, UserPlus } from "lucide-react"
...
export default function AdminCustomersPage() {
  ...
  const filteredCustomers = customers.filter((customer) => {
    ...
    const matchesSearch = name.includes(searchQuery.toLowerCase()) || email.includes(searchQuery.toLowerCase()) || phone.includes(searchQuery)
  ...
  const sortedCustomers = [...filteredCustomers].sort((a, b) => {
    switch (sortBy) {
      ...
      default:
        return new Date(b.joinDate || b.createdAt || 0).getTime() - new Date(a.joinDate || a.createdAt || 0).getTime()
    }
  })
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Theo dõi và quản lý thông tin khách hàng</p>
              </div>
            </div>
      ...
      <div className="p-6">
        ...
        <Card className="border-slate-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-slate-900">Danh sách khách hàng ({sortedCustomers.length})</CardTitle>
              <div className="text-sm text-slate-600">
                Trung bình chi tiêu:{" "}
                {customers.length > 0
                  ? (customers.reduce((sum, c) => sum + c.totalSpent, 0) / customers.length).toLocaleString("vi-VN")
                  : 0}
                đ
              </div>
            </div>
          ...
          <CardContent>
                ...
                <TableBody>
                  {sortedCustomers.map((customer) => (
                    <TableRow key={customer.id}>
                      ...
                      <TableCell>
                        <p className="text-sm text-slate-900 line-clamp-2">{customer.address}</p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-900">{customer.joinDate}</p>
                      </TableCell>
                      <TableCell>
                        <p className="font-medium text-slate-900">{customer.totalOrders}</p>
                      </TableCell>
                      <TableCell>
                        <p className="font-medium text-slate-900">{customer.totalSpent.toLocaleString("vi-VN")}đ</p>
                      </TableCell>
                      <TableCell>
                        <p className="text-sm text-slate-900">{customer.lastOrder || "Chưa có"}</p>
                      </TableCell>
            ...
            {sortedCustomers.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Không tìm thấy khách hàng</h3>
