...
export default function NewProductPage() {
    ...
    try {
      ...
      try {
        ...
        const res = await fetch('/api/local-products', {
          ...
          body: JSON.stringify({ product: { title, description, price, inStock: stock }, images: b64s }),
        })
        const payload = await (async () => {
          try { return await res.json() } catch (e) { return null }
        })()
        if (res.ok) {
          setMessage('Đã lưu sản phẩm (local server). Sản phẩm sẽ hiển thị cho mọi người.')
  ...
  try { window.localStorage.setItem('wb_products_update', Date.now().toString()) } catch (e) { /* ignore */ }
  setMessage('Đã lưu sản phẩm vào bộ nhớ trình duyệt (local). Cấu hình Firestore để mọi người thấy sản phẩm này.')
  router.push('/products')
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white shadow rounded p-6">
          ...
          {message && (
            <div className="mb-4 p-3 rounded bg-rose-50 text-rose-700">{message}</div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="mt-4">
                ...
                <textarea className="mt-1 block w-full border rounded px-3 py-2" value={description} onChange={(e) => setDescription(e.target.value)} rows={4} />
              </div>

              <div className="grid grid-cols-2 gap-4 mt-2">
                <div>
                  <label className="block text-sm font-medium">Giá</label>
                  <input type="number" className="mt-1 block w-full border rounded px-3 py-2" value={price as any} onChange={(e) => setPrice(e.target.value === '' ? '' : Number(e.target.value))} required />
                </div>
                <div>
                  <label className="block text-sm font-medium">Tồn kho</label>
                  <input type="number" className="mt-1 block w-full border rounded px-3 py-2" value={stock as any} onChange={(e) => setStock(e.target.value === '' ? '' : Number(e.target.value))} required />
                </div>
              </div>

              <input ref={fileInputRef} type="file" accept="image/*" multiple className="mt-2" onChange={onFilesSelected} />
              <div className="mt-3 grid grid-cols-4 gap-3">
                {files.map((f, i) => (
                  <div key={i} className="relative">
                    <img src={URL.createObjectURL(f)} alt={f.name} className="w-full h-24 object-cover rounded" />
                    <button type="button" onClick={() => removeImage(i)} className="absolute top-1 right-1 bg-white rounded-full p-1">✕</button>
                    {progressMap[f.name] !== undefined && (
                      <div className="absolute left-0 bottom-0 w-full bg-black/40 text-white text-xs text-center py-0.5">{progressMap[f.name]}%</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="flex items-center gap-3">
              <button type="submit" disabled={isSubmitting} className="bg-rose-500 text-white px-4 py-2 rounded">{isSubmitting ? 'Đang lưu...' : 'Lưu'}</button>
              <button type="button" onClick={() => router.push('/admin/products')} className="border px-3 py-2 rounded">Hủy</button>
            </div>
          </form>
        </div>
      </div>
