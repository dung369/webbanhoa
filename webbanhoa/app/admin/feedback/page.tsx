          ...
          <CardContent>
            {loading ? <div>Đang tải...</div> : (
              <div className="space-y-4">
                {items.length === 0 ? <div>Chưa có phản hồi</div> : items.map((it) => (
                  <div key={it.id} className="border rounded p-4 bg-white">
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="font-semibold text-slate-900">{it.name || it.email || 'Khách'}</div>
                        <div className="text-sm text-slate-600">{it.email}</div>
                        <div className="text-xs text-slate-500">{it.createdAt}</div>
                      </div>
                      <div className="space-x-2">
                        <Button variant="ghost" onClick={() => handleDelete(it.id)}>Xóa</Button>
                      </div>
                    </div>
