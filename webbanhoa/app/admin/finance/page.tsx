...
import { ArrowLeft, TrendingUp, DollarSign, CreditCard, Calendar, Download, Filter, Plus } from "lucide-react"
...
export default function FinancePage() {
  ...
  const pendingRevenue = orders.filter((o) => ['pending', 'confirmed', 'preparing', 'shipping'].includes(o.status)).reduce((s, o) => s + (Number(o.total) || 0), 0)
  ...
  useEffect(() => {
    try {
      ...
      const arr = Object.keys(catMap).map((k, i) => ({ name: k, value: catMap[k], color: palette[i % palette.length] }))
      setCategoryRevenue(arr.length ? arr : [
  ...
  const transactions = orders.map((order) => ({
    id: order.id,
    type: order.status === 'delivered' ? 'income' : order.status === 'cancelled' ? 'refund' : 'pending',
    amount: Number(order.total) || 0,
    description: `Đơn hàng #${order.id}`,
    date: order.createdAt && order.createdAt.toDate ? order.createdAt.toDate() : (order.createdAt ? new Date(order.createdAt) : new Date()),
    status: order.status,
    paymentMethod: order.paymentMethod,
  }))

  const filteredTransactions = transactions.filter(
    (transaction) => transaction.id.toString().toLowerCase().includes(searchQuery.toLowerCase()) || transaction.description.toLowerCase().includes(searchQuery.toLowerCase()),
  )

  const getTransactionBadge = (type: string, status: string) => {
    if (type === "income") return <Badge className="bg-green-100 text-green-700">Thu nhập</Badge>
    if (type === "refund") return <Badge className="bg-red-100 text-red-700">Hoàn tiền</Badge>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Theo dõi doanh thu và chi phí</p>
              </div>
            </div>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{averageOrderValue.toLocaleString("vi-VN")}₫</div>
              <p className="text-xs text-muted-foreground">
                <TrendingUp className="inline h-3 w-3 mr-1 text-green-500" />
                +5% từ tháng trước
              </p>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{pendingRevenue.toLocaleString("vi-VN")}₫</div>
              <p className="text-xs text-muted-foreground">
                Từ {orders.filter((o: any) => !["delivered", "cancelled"].includes(o.status)).length} đơn hàng
              </p>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Phân bố doanh thu theo loại hoa</CardDescription>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                  ...
                  {products.slice(0, 5).map((product, index) => (
                    <div key={product.id || index} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium">{product.name || `Sản phẩm ${index + 1}`}</p>
                          <p className="text-sm text-muted-foreground">{product.category || 'Khác'}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-medium">
                          {(Number(product.price || 0) * Math.floor(Math.random() * 20 + 5)).toLocaleString("vi-VN")}₫
                        </p>
                        <p className="text-sm text-muted-foreground">{Math.floor(Math.random() * 20 + 5)} đã bán</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="transactions" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Danh sách tất cả giao dịch tài chính</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {filteredTransactions.map((transaction) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell>{transaction.date.toLocaleDateString("vi-VN")}</TableCell>
                        <TableCell>
                          <Badge variant="outline">
                            {transaction.paymentMethod === "COD" ? "Tiền mặt" : "Chuyển khoản"}
                          </Badge>
                        </TableCell>
                        <TableCell>{getTransactionBadge(transaction.type, transaction.status)}</TableCell>
          ...
          <TabsContent value="analytics" className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Phân bố phương thức thanh toán</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Các chỉ số quan trọng về tài chính</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-2xl font-bold text-green-600">
                      {((totalRevenue / (totalRevenue + 5000000)) * 100).toFixed(1)}%
                    </div>
                    <p className="text-sm text-muted-foreground">Tỷ suất lợi nhuận</p>
                  </div>
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">
                      {((totalRevenue / totalOrders) * 0.15).toLocaleString("vi-VN")}₫
                    </div>
                    <p className="text-sm text-muted-foreground">Chi phí TB/đơn hàng</p>
                  </div>
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">
                      {(totalRevenue / 30).toLocaleString("vi-VN")}₫
                    </div>
                    <p className="text-sm text-muted-foreground">Doanh thu TB/ngày</p>
                  </div>
                </div>
              </CardContent>
            </Card>
