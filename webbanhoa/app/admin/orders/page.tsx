...
import { ArrowLeft, Search, Eye, Edit, Truck, Filter, Download } from "lucide-react"
...
export default function AdminOrdersPage() {
      ...
      try {
        ...
        unsub = onSnapshot(q, (snap) => {
          const items = snap.docs.map((d) => {
            ...
            return {
              id: d.id,
              customer: data.customerName || (user ? `${user.firstName || ''} ${user.lastName || ''}` : 'Unknown'),
              email: data.email || (user && user.email) || 'unknown@email.com',
              phone: data.phone || (user && user.phone) || 'N/A',
              total: Number(data.total || 0),
              status: data.status || 'pending',
              paymentMethod: data.paymentMethod || 'COD',
              date: data.createdAt && typeof data.createdAt.toDate === 'function' ? data.createdAt.toDate().toISOString().split('T')[0] : (data.createdAt ? new Date(data.createdAt).toISOString().split('T')[0] : ''),
              items: Array.isArray(data.items) ? data.items.length : (data.itemsCount || 0),
              address: data.shippingAddress ? `${data.shippingAddress.address}, ${data.shippingAddress.ward}, ${data.shippingAddress.district}, ${data.shippingAddress.city}` : '',
            }
          })
          setOrders(items)
        })
  ...
  const markDelivered = async (orderId: string) => {
    try {
      await fetch('/api/orders', { method: 'PUT', body: JSON.stringify({ id: orderId, status: 'delivered' }), headers: { 'Content-Type': 'application/json' } })
      // refresh local list
      const res = await fetch('/api/orders')
      const data = await res.json()
      if (data && data.orders) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">Theo dõi và xử lý đơn hàng</p>
              </div>
            </div>
      ...
      <div className="p-6">
        ...
        <Card className="border-slate-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-slate-900">Danh sách đơn hàng ({sortedOrders.length})</CardTitle>
              <div className="text-sm text-slate-600">
                Tổng giá trị: {sortedOrders.reduce((sum, order) => sum + order.total, 0).toLocaleString("vi-VN")}đ
              </div>
            </div>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {sortedOrders.map((order) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell>
                        <p className="text-sm text-slate-900">{order.items} sản phẩm</p>
                      </TableCell>
                      <TableCell>
                        <p className="font-medium text-slate-900">{order.total.toLocaleString("vi-VN")}đ</p>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          {order.status !== 'delivered' && (
                            <Button variant="ghost" size="sm" className="text-green-800 hover:text-green-900" onClick={() => markDelivered(order.id)}>
                              Đã giao & trả tiền
                            </Button>
                          )}
                        </div>
            ...
            {sortedOrders.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">Không tìm thấy đơn hàng</h3>
