...
export async function PUT(req: Request) {
  try {
    ...
    const order = arr[idx]
    const existing = customers.find((c: any) => c.email === (order.email || ''))
    if (existing) {
      existing.totalOrders = (existing.totalOrders || 0) + 1
      existing.totalSpent = (existing.totalSpent || 0) + Number(order.total || 0)
      existing.lastOrder = order.createdAt
    } else {
      customers.push({ id: `cust_${Date.now()}`, name: order.fullname || '', email: order.email || '', phone: order.phone || '', totalOrders: 1, totalSpent: Number(order.total || 0), lastOrder: order.createdAt })
    }
