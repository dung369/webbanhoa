...
export async function POST(req: Request) {
  try {
    ...
    const { userId, senderId, recipientId, name, email, message, role } = body as any
    const finalMessage = message || body?.text || null
    if (!finalMessage) return NextResponse.json({ error: 'missing message' }, { status: 400 })
...
export async function DELETE(req: Request) {
  try {
    ...
    if (firestore) {
      ...
      return NextResponse.json({ error: 'delete not supported for Firestore via this route' }, { status: 400 })
    }
