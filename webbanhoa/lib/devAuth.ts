...
type DevUser = { uid: string; email?: string | null; displayName?: string | null }
...
export async function register(email: string, password: string) {
  ...
  const isSeedAdmin = email === process.env.SEED_ADMIN_EMAIL || email === 'trandaidung9a1@gmail.com'
  devUser = { uid: isSeedAdmin ? `admin-${Date.now()}` : `dev-${Date.now()}`, email, displayName: isSeedAdmin ? 'Admin' : null }
  notify(devUser)
  return { user: devUser }
}
...
export async function isAdminUser(u: any) {
  ...
  if (firebaseGetIdTokenResult) {
    try {
      ...
      if (claims && (claims.role === 'admin' || claims.admin === true)) return true
    } catch (e) {
  ...
  if (u.email && (u.email === process.env.SEED_ADMIN_EMAIL || u.email === 'trandaidung9a1@gmail.com')) return true
  return false
}
